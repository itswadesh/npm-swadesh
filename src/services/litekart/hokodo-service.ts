...
export const getCreditLimit = async ({
  ...
  if (company_id) {
    ...
    res = await getAPI(
      `hokodo-onboarding/get-credit-limit?company_id=${company_id}&organisation_id=${organisation_id}&store=${storeId}`,
      origin,
      sid
    )
  }
...
export const getOrderDetailAfterPayment = async ({
  ...
  res = await getAPI(
    `hokodo-checkout/order-detail-after-payment?hokodo_order_id=${hokodo_order_id}&store=${storeId}`,
    origin,
    sid
  )
...
export const getPaymentStatus = async ({
  ...
  res = await getAPI(
    `hokodo-checkout/payment-status?payment_id=${payment_id}&store=${storeId}`,
    origin,
    sid
  )
